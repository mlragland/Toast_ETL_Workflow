#!/usr/bin/env python3
"""
Fixed Toast ETL Dashboard with correct BigQuery schema.
"""
import os
import sys
    ...
    try:
        ...
        for table in tables:
            try:
                ...
                row_count = int(result.iloc[0]['row_count']) if len(result) > 0 else 0
...
def dashboard_summary():
    ...
    try:
        ...
        SELECT 
            ...
            COALESCE(AVG(CAST(tax AS FLOAT64)), 0) as avg_tax
        ...
        items_query = """
        SELECT 
            COALESCE(SUM(CAST(total_price AS FLOAT64)), 0) as total_revenue
        FROM `toast-analytics-444116.toast_analytics.all_items_report`
        WHERE processing_date = '2025-06-11'
        """
        
        items_result = client.query(items_query).to_dataframe()
        total_revenue = float(items_result.iloc[0]['total_revenue']) if len(items_result) > 0 else 0
...
def recent_orders():
    ...
    try:
        ...
        SELECT 
            customer_id,
            COALESCE(CAST(tax AS FLOAT64), 0) as amount,
            opened_date,
            opened_time,
            'completed' as status
        FROM `toast-analytics-444116.toast_analytics.check_details`
        ...
        for _, row in results.iterrows():
            orders.append({
                "id": row['customer_id'] if row['customer_id'] else f"order-{len(orders)+1}",
...
def top_servers():
    ...
    try:
        ...
        SELECT 
            server as name,
            COUNT(*) as orders,
            COALESCE(SUM(CAST(tax AS FLOAT64)), 0) as sales
        FROM `toast-analytics-444116.toast_analytics.check_details`
        WHERE processing_date = '2025-06-11'
        AND server IS NOT NULL
        AND server != ''
...
def sales_by_service():
    ...
    try:
        ...
        SELECT 
            ...
            COALESCE(SUM(CAST(total_price AS FLOAT64)), 0) as sales,
            COUNT(*) as items
        FROM `toast-analytics-444116.toast_analytics.all_items_report`
        WHERE processing_date = '2025-06-11'
...
if __name__ == '__main__':
    print("🍴 Toast ETL Fixed Dashboard")
    print("=" * 50)
